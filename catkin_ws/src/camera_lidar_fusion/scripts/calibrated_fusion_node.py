...
class CalibratedFusionNode:
    def __init__(self):
        rospy.init_node('calibrated_fusion_node', anonymous=True)
        ...
        # Calibrated parameters based on your setup
        ...
        # Camera parameters (adjusted based on calibration)
        ...
        self.lidar_fov_horizontal = 179.9  # degrees (from calibration)
        self.lidar_fov_vertical = 179.9    # degrees (from calibration)
        ...
        rospy.loginfo("Calibrated Fusion Node initialized")
        rospy.loginfo("Camera FOV: %.1f° x %.1f°", self.camera_fov_horizontal, self.camera_fov_vertical)
        rospy.loginfo("LiDAR FOV: %.1f° x %.1f°", self.lidar_fov_horizontal, self.lidar_fov_vertical)
    ...
    def image_to_lidar_coordinates(self, image_x, image_y):
        ...
        # Calculate angles from center (camera FOV)
            ...
            for i in range(0, len(self.latest_cloud.data), point_step):
                if i + point_step <= len(self.latest_cloud.data):
                    ...
                    if z_cam > 0:  # Only consider points in front of camera
                        ...
                        # Use wider tolerance since LiDAR has much larger FOV
                        angle_tolerance = math.radians(10.0)  # 10 degree tolerance
    ...
    def image_callback(self, image_msg):
        ...
        try:
            ...
            # Add system info
            cv2.putText(debug_image, "Calibrated LiDAR-Camera Fusion", (10, 30), 
            ...
            cv2.putText(debug_image, "Camera FOV: %.1f x %.1f" % 
                       (self.camera_fov_horizontal, self.camera_fov_vertical), 
                       (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)
            
            cv2.putText(debug_image, "LiDAR FOV: %.1f x %.1f" % 
                       (self.lidar_fov_horizontal, self.lidar_fov_vertical), 
            ...
            if self.frame_count % 30 == 0:
                rospy.loginfo("Calibrated Fusion: Processed %d frames, detected %d objects", 
